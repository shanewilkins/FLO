...
_PADDING = 24.0
_LANE_COLORS = ("#eff6ff", "#f0fdf4", "#fff7ed", "#f8fafc", "#fdf2f8")
...
def render_swimlane_svg_artifact(
    ...
    for index, lane in enumerate(result.lanes):
        parts.extend(_lane_svg(lane=lane, index=index))

    for edge_key in sorted(result.edge_paths.keys()):
        parts.extend(_edge_svg(result.edge_paths[edge_key]))
...
def _lane_svg(*, lane: Any, index: int) -> list[str]:
    bounds = lane.bounds
    fill = _LANE_COLORS[index % len(_LANE_COLORS)]
    lane_id = escape(str(lane.id))
    label = escape(str(lane.label or lane.id))
    return [
        f'<g data-lane-id="{lane_id}">',
        f'<rect x="{bounds.x_px:.1f}" y="{bounds.y_px:.1f}" width="{bounds.width_px:.1f}" height="{bounds.height_px:.1f}" '
        f'rx="14" fill="{fill}" stroke="#cbd5e1" stroke-width="1.5" />',
        f'<text x="{bounds.x_px + 14.0:.1f}" y="{bounds.y_px + 24.0:.1f}" font-family="Helvetica" font-size="13" font-weight="700" fill="#334155">{label}</text>',
        "</g>",
    ]
...
def _edge_svg(edge_path: Any) -> list[str]:
    ...
    if edge_path.label:
        ...
        label = escape(str(edge_path.label))
        parts.append(
            f'<rect x="{placement.x - 12.0:.1f}" y="{placement.y - 12.0:.1f}" width="24.0" height="16.0" rx="6" fill="white" fill-opacity="0.92" />'
        )
        parts.append(
            f'<text x="{placement.x:.1f}" y="{placement.y:.1f}" text-anchor="{placement.anchor}" font-family="Helvetica" font-size="12" fill="#0f172a">{label}</text>'
        )
    parts.append("</g>")
    return parts
