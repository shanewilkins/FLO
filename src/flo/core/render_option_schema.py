...
_RENDER_OPTION_SPECS: tuple[RenderOptionSpec, ...] = (
    ...
    RenderOptionSpec(
        "profile",
        "--profile",
        "Projection rule profile",
        choices=("default", "analysis"),
    ),
    RenderOptionSpec(
        "detail", "--detail", "Detail level", choices=("summary", "standard", "verbose")
    ),
    RenderOptionSpec(
        "orientation",
        "--orientation",
    ...
    RenderOptionSpec(
        "show_notes", "--show-notes", "Include node notes in DOT labels", is_flag=True
    ),
    RenderOptionSpec(
        "no_header", "--no-header", "SPPM publication: hide header band", is_flag=True
    ),
    RenderOptionSpec(
        "no_footer", "--no-footer", "SPPM publication: hide footer band", is_flag=True
    ),
    RenderOptionSpec(
        "subprocess_view",
        "--subprocess-view",
        "Subprocess rendering mode",
        choices=("expanded", "parent-only"),
    ),
    RenderOptionSpec(
        "sppm_projection",
        "--sppm-projection",
        "SPPM publication: hierarchy projection mode",
        choices=("top-level", "child-map", "inline"),
    ),
    RenderOptionSpec(
        "sppm_focus_subprocess",
        "--sppm-focus-subprocess",
        "SPPM publication: subprocess node id to focus for child-map/inline",
    ),
    RenderOptionSpec(
        "spaghetti_channel",
        "--spaghetti-channel",
        "Movement channel for spaghetti diagrams",
    ...
    RenderOptionSpec(
        "sppm_step_numbering",
        "--sppm-step-numbering",
        "SPPM step numbering mode",
        choices=("off", "node", "edge"),
    ),
    RenderOptionSpec(
        "sppm_label_density",
        "--sppm-label-density",
        "SPPM label density mode",
        choices=("full", "compact", "teaching"),
    ),
    RenderOptionSpec(
        "sppm_wrap_strategy",
        "--sppm-wrap-strategy",
        "Text wrapping strategy for SPPM labels",
        choices=("word", "balanced", "hard"),
    ),
    RenderOptionSpec(
        "sppm_truncation_policy",
        "--sppm-truncation-policy",
        "Label truncation policy for SPPM text",
    ...
    RenderOptionSpec(
        ...
        "Target columns/steps per wrapped chunk",
        value_type=int,
    ),
    RenderOptionSpec(
        "sppm_max_label_step_name",
        "--sppm-max-label-step-name",
        "Max step-name label length for SPPM",
        value_type=int,
    ),
    RenderOptionSpec(
        "sppm_max_label_workers",
        "--sppm-max-label-workers",
        "Max workers label length for SPPM",
        value_type=int,
    ),
    RenderOptionSpec(
        "sppm_max_label_ctwt",
        "--sppm-max-label-ctwt",
        "Max CT/WT label length for SPPM",
        value_type=int,
    ),
    RenderOptionSpec(
        "sppm_output_profile",
        "--sppm-output-profile",
        "SPPM publication: output profile preset",
...
def iter_render_option_specs(
    *, include_render_to: bool = True
) -> tuple[RenderOptionSpec, ...]:
...
def add_argparse_render_options(
    parser: argparse.ArgumentParser, *, include_render_to: bool = True
) -> None:
...
def build_render_options_from_namespace(
    parsed: object, *, include_render_to: bool = True
) -> dict[str, Any]:
