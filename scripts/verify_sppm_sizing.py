...
def _append_rectangle_report(report: list[str], rectangles: list[SVGElement], spec: SizingSpec) -> bool:
    all_passed = True
    for rect in rectangles:
        elem_id = rect.elem_id or "(no id)"
        status, passed = _rectangle_status(rect, spec)
        all_passed = all_passed and passed
        report.append(f"{elem_id:<20} {'rect':<12} {rect.width or 0:<12.1f} {rect.height or 0:<12.1f} {status:<15}")
    return all_passed


def _append_circle_report(report: list[str], circles: list[SVGElement], spec: SizingSpec) -> bool:
    all_passed = True
    for circle in circles:
        ...
        all_passed = all_passed and passed
        report.append(f"{elem_id:<20} {circle.elem_type:<12} {diameter:<12.1f} {'-':<12} {status:<15}")
    return all_passed
...
def _append_polygon_report(report: list[str], polygons: list[SVGElement]) -> None:
    ...
    report.append("  Note: SPPM nodes use HTML-like tables (polygons) for complex labels.")
    ...
    for polygon in polygons[:10]:
        ...
        report.append(f"{elem_id:<20} {'polygon':<12} {polygon.width or 0:<12.1f} {polygon.height or 0:<12.1f} {'~ (cell)':<15}")
    if len(polygons) > 10:
...
def _append_summary(report: list[str], *, all_passed: bool, rectangles: list[SVGElement], circles: list[SVGElement], polygons: list[SVGElement], spec: SizingSpec) -> None:
    report.append(f"\n{'-'*71}")
    report.append("\nSummary:")
...
def verify_sizing(svg_path: Path, dpi: int = 96) -> tuple[bool, list[str]]:
    ...
    circles = [element for element in unique_elements if element.elem_type in ("circle", "ellipse")]
    rectangles = [element for element in unique_elements if element.elem_type == "rect"]
    polygons = [element for element in unique_elements if element.elem_type == "polygon"]
