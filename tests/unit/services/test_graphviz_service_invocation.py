"""Unit tests for Graphviz subprocess invocation and format handling."""
...
def test_dot_subprocess_success(tmp_path, monkeypatch):
    ...
    with patch(
        "flo.services.graphviz.subprocess.run", return_value=mock_result
    ) as mock_run:
        render_dot_to_file(SIMPLE_DOT, out)
...
def test_dot_subprocess_oserror_raises_render_error(tmp_path, monkeypatch):
    ...
    with patch(
        "flo.services.graphviz.subprocess.run", side_effect=OSError("permission denied")
    ):
